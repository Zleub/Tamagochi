...
* @Last Modified time: 2015-11-17 17:47:34
*)

exception Exit of Gameplay.modList
...
let init () =
	...
	let rec run time hamtaro modList =
		...
		| Sdlevent.KEYDOWN { Sdlevent.keysym = Sdlkey.KEY_ESCAPE } -> raise (Exit modList)
		| Sdlevent.QUIT -> raise (Exit modList)
		| _ -> () in
	...
	| Exit modList -> Gameplay.save modList
