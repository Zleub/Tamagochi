...
* @Last Modified time: 2015-11-16 19:15:59
*)

exception Lost

class virtual base ?state renderer font label position =
object (self)

	val _state = match state with | Some x -> x | None -> 100
...
class health tmp renderer font =
object (self)
	inherit base ~state:tmp renderer font "Health" (30, 10) as super
...
class energy tmp renderer font =
object (self)
	inherit base ~state:tmp renderer font "Energy" (160, 10) as super
...
class hygiene tmp renderer font =
object (self)
	inherit base ~state:tmp renderer font "Hygiene" (290, 10) as super
...
class happyness tmp renderer font =
object (self)
	inherit base ~state:tmp renderer font "Happyness" (420, 10) as super
